...
Original file is located at
    https://colab.research.google.com/drive/1LHaNOI-GVoV4pKSnm4J5rL8pe6G2sPxH
"""
...
faces_csv=path+'/faces.csv'
train_df, valid_df, test_df = train_valid_test_split(faces_csv)
...
class Faces(Dataset):
    def __init__(self, dataset, width, height, dir_path="/root/.cache/kagglehub/datasets/sbaghbidi/human-faces-object-detection/versions/1/images"):
...
for epoch in range(NUM_EPOCHS):
    ...
    if (epoch+1) == NUM_EPOCHS: # save loss plots and model once at the end
        ...
        figure_1.savefig(f"/kagglehub {epoch+1}.png")
        figure_2.savefig(f"/kagglehub{epoch+1}.png")
        torch.save(model.state_dict(), f"/kagglehub{epoch+1}.pth")
...
def add_caption(image, text="Human"):
    # Create black bar at bottom
    h, w = image.shape[:2]
    caption_height = 40  # Adjust based on text size
    new_image = np.zeros((h + caption_height, w, 3), dtype=np.uint8)
    new_image[:h, :w] = image
    ...
    text_y = h + int(caption_height * 0.7)

    # Add text
    cv2.putText(new_image, text, (text_x, text_y),
...
# get all the image names in sorted order
dir_path="/root/.cache/kagglehub/datasets/sbaghbidi/human-faces-object-detection/versions/1/images"
    ...
    if len(outputs[0]['boxes']) != 0:
        ...
        for j, box in enumerate(draw_boxes):
            cv2.rectangle(orig_image,
                        ...
                        (int(box[2]), int(box[3])),
                        (0, 0, 255), 2)
        final_image = add_caption(orig_image)
